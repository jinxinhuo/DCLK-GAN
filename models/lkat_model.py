...
class Decoder2(nn.Module):
    def __init__(self, in_planes=1, out_planes=16):
        ...
        self.dcn1 = DCNv3_pytorch(channels=64, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
                                  act_layer='GELU', norm_layer='LN', center_feature_scale=False)
        self.ct1 = Atten(out_planes*2)
        self.dcn2 = DCNv3_pytorch(channels=128, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
                                  act_layer='GELU', norm_layer='LN', center_feature_scale=False)
        self.ct2 = Atten(out_planes*4)
        self.dcn3 = DCNv3_pytorch(channels=32, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
                                  act_layer='GELU', norm_layer='LN', center_feature_scale=False)
        self.ct3 = Atten(out_planes*8)
        # self.dcn4 = DCNv3_pytorch(channels=64, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
        #                           act_layer='GELU', norm_layer='LN', center_feature_scale=False)
        # self.dcn5 = DCNv3_pytorch(channels=128, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
        #                           act_layer='GELU', norm_layer='LN', center_feature_scale=False)
        # self.dcn6 = DCNv3_pytorch(channels=256, kernel_size=3, stride=1, pad=1, dilation=1, group=4, offset_scale=1.0,
        #                           act_layer='GELU', norm_layer='LN', center_feature_scale=False)
    def forward(self,input):
        x1=self.ct1(self.d(input))
        x11 = self.dcn2(x1)
        x2=self.ct2(self.d2(x11))
        x22=self.dcn1(x2)
        x3=self.ct3(self.d3(x22))
        x33=self.dcn3(x3)
        x41=self.up3(F.interpolate(x33, scale_factor=2, mode="nearest"))+x22
        #x42=self.dcn4(x41) #这些卷积效果不好
        x51=self.up2(F.interpolate(x41, scale_factor=2, mode="nearest"))+x11
        #x52 = self.dcn5(x51)
        x61=self.up(F.interpolate(x51, scale_factor=2, mode="nearest"))
        #x62 = self.dcn6(x61)
        return x61
...
class SpatialGroupEnhance(nn.Module):
    ...
    def __init__(self, groups):
        ...
        self.sig = nn.Sigmoid()
        self.init_weights()
        self.mlp1 = nn.Conv2d(1, 1, 1, 1, 0)
        self.mlp2 = nn.Conv2d(64, 64, 1, 1, 0)
...
class LKAT_network(nn.Module):
    def __init__(self,in_planes=3,out_planes=64):
        ...
        self.SGE = SpatialGroupEnhance(groups=64)
    ...
    def forward(self,nir_image):
        ...
        output = self.SGE(f_rgb_ence)
        #f=torch.cat((d2,f_rgb_ence),dim=1)
        f = torch.cat((d2, output), dim=1)
        f_rgb = self.out(f)
        return f_rgb
...
class lkatModel(BaseModel):
    ...
    def get_ssim(self,img1, img2):
        return structural_similarity(img1, img2, multichannel = True, win_size = 3)
