...
if __name__ == '__main__':
    ...
    opt2.phase += 'test_monti'

    opt3 = TestOptions().parse()
    opt3.phase += 'test_traffic'

    data_loader = CreateDataLoader(opt)
    dataset = data_loader.load_data()

    data_loader_test_monti = CreateDataLoader(opt2)
    dataset_test_monti = data_loader_test_monti.load_data()

    output_freq = 100


    data_loader_test_traffic = CreateDataLoader(opt3)
    dataset_test_traffic = data_loader_test_traffic.load_data()

    dataset_size = len(data_loader)
    dataset_size_test_monti = len(data_loader_test_monti)
    dataset_size_test_traffic = len(data_loader_test_traffic)
    print('#training images = %d' % dataset_size)
    print('#test images monti = %d' % dataset_size_test_monti)
    print('#test images traffic = %d' % dataset_size_test_traffic)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        with tqdm(total=math.ceil(len(dataset)/opt.batchSize), ascii=True) as tt:
            ...
            for i, data in enumerate(dataset):
                ...
                #     cv2.imwrite(Train_save_img_path + '/train_' + str(epoch) + '_' + str(i+1) + ".png",
                #                 color_img_sum)
                #
                ...
                #     # print(img_num)
                #
                #     f = open(os.path.join(model_test_result_path, 'resultEPSA_ACmixUnet_gll.csv'), 'a',newline='')  #把w改成a就可以不覆盖原有数据
                #
                #     # 2. 基于文件对象构建 csv写入对象
            ...
            ssim_avg = ssim_sum / img_num
            psnr_avg = psnr_sum / img_num
            f2 = open('./checkpoints/each_epoch_LKATGAN_train.csv', 'a', newline='')  # 把w改成a就可以不覆盖原有数据
            # 2. 基于文件对象构建 csv写入对象
        ...
        img_test_num = 0
        if epoch % 1 == 0:
            with tqdm(total=len(dataset_test_monti), ascii=True) as tt2:
                tt2.set_description('cal Test PSNR')
                epoch_start_time = time.time()
                iter_data_time = time.time()
                epoch_iter = 0
                for i, data in enumerate(dataset_test_monti):
                    ...
                    if ((i+1) % output_freq == 0):
                        ...
                        cv2.imwrite(
                            r"./checkpoints/test_img/" + 'Monti_test_' + str(epoch) + '_' + str(i + 1) + ".png",
                            color_img_sum2)

                        message = '(dataset: monti, epoch: %d, iters: %d, test_ssim: %.5f,test_psnr:%.5f) ' % (
                ...
                print("TestPSNR:")
                print(psnr_test_avg)
                f3 = open('./checkpoints/each_epoch_LKATGAN_test.csv', 'a', newline='')  # 把w改成a就可以不覆盖原有数据
                # 2. 基于文件对象构建 csv写入对象
                csv_writer = csv.writer(f3)
                csv_writer.writerow(['monti:' ,epoch, ssim_test_avg, psnr_test_avg])  # 这里是写入一个epoch最终的数据
                f3.close()



            # 保存当前traffic训练和测试的结果
            ssim_test_sum_traffic = 0
            psnr_test_sum_traffic = 0
            img_test_num_traffic = 0
            if epoch % 1 == 0:
                with tqdm(total=len(dataset_test_traffic), ascii=True) as tt2:
                    tt2.set_description('cal Test PSNR')
                    epoch_start_time = time.time()
                    iter_data_time = time.time()
                    epoch_iter = 0
                    for i, data in enumerate(dataset_test_traffic):
                        # if i>10000 and epoch < 15:
                        #     break
                        img_test_num_traffic += 1
                        model.set_input(data)
                        model.test()
                        ...
                        ssim_test = cal_ssim(labels, result)
                        psnr_test = cal_psnr(labels, result)
                        ssim_test_sum_traffic = ssim_test_sum_traffic + ssim_test
                        psnr_test_sum_traffic = psnr_test_sum_traffic + psnr_test
                        ...
                        if ((i + 1) % output_freq == 0):
                            ...
                            color_img_sum2 = cv2.cvtColor(color_img_sum2, cv2.COLOR_BGR2RGB)
                            cv2.imwrite(
                                r"./checkpoints/test_img/" + 'Traffic_test_' + str(epoch) + '_' + str(i + 1) + ".png",
                                color_img_sum2)

                            message = '(dataset: traffic, epoch: %d, iters: %d, test_ssim: %.5f,test_psnr:%.5f) ' % (
                                epoch, img_test_num_traffic, ssim_test_sum_traffic / img_test_num_traffic,
                                psnr_test_sum_traffic / img_test_num_traffic)
                            print(message)

                    ssim_test_avg = ssim_test_sum_traffic / img_test_num_traffic
                    psnr_test_avg = psnr_test_sum_traffic / img_test_num_traffic

                    print("TestSSIM:")
                    print(ssim_test_avg)
                    print("TestPSNR:")
                    print(psnr_test_avg)
                    f4 = open('./checkpoints/each_epoch_LKATGAN_test.csv', 'a', newline='')  # 把w改成a就可以不覆盖原有数据
                    # 2. 基于文件对象构建 csv写入对象
                    csv_writer = csv.writer(f4)
                    csv_writer.writerow(['traffic:', epoch, ssim_test_avg, psnr_test_avg])  # 这里是写入一个epoch最终的数据
                    f4.close()
